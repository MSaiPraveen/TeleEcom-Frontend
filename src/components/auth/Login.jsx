...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // axios base is expected to include /api (VITE_API_URL). We call the relative path here.
  ...
  const normalizeBase = (s = "") => {
    if (!s) return "";
    return clean(s).replace(/\/api$/i, "");
  };

  const BACKEND_ROOT = normalizeBase(RAW_BASE) || normalizeBase(RAW_API) || "https://springteleecom.onrender.com";
  const oauthHref = (provider) =>
    USE_PROXY ? `/oauth2/authorization/${provider}` : `${BACKEND_ROOT}/oauth2/authorization/${provider}`;
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-slate-50 dark:bg-slate-900">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-2 mb-8">
            ...
            <span className="text-2xl font-bold text-slate-900 dark:text-white">TeleEcom</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" className="w-full" type="button" onClick={handleGoogleLogin} disabled={loading} aria-label="Sign in with Google">
            ...
            <Button variant="outline" className="w-full" type="button" onClick={handleGithubLogin} disabled={loading} aria-label="Sign in with GitHub">
